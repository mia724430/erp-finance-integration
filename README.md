...
> **Status: Week 1, Day 3** — ERP.Simulator generates fake invoice/order CSVs; Integration.Worker polls for them and parses new files. Transform/database steps not built yet.
...
`Integration.Worker` polls `data/incoming/` every 5 seconds, parses any CSV files it finds, logs how many invoices it read, and moves each file into `data/processed/` so it isn't picked up again.

Transformation to Xero format and the database write land Week 1, Days 4–6.
